...
"""Qwen3 parsers for tool calls and reasoning.

Two parsers are provided:

* ``Qwen3XMLParser`` — tool calls only (XML ``<tool_call>`` format)
* ``Qwen3Parser`` — reasoning (``<think>``/``</think>``) **plus**
...
@functools.cache
def qwen3xml_config() -> ParserEngineConfig:
...
class Qwen3XMLParser(ParserEngine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            tokenizer, tools, parser_engine_config=qwen3xml_config(), **kwargs
        )
