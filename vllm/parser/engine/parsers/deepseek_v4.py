...
class DeepSeekV4Parser(ParserEngine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if tools:
            self._base_arg_converter = self._arg_converter
            self._arg_converter = self._unwrapping_arg_converter

    def _unwrapping_arg_converter(self, raw_args: str, partial: bool) -> str:
        assert self._base_arg_converter is not None
        result = self._base_arg_converter(raw_args, partial)
        func_name = self._tool_slots[-1].name if self._tool_slots else None
