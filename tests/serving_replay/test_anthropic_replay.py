...
from tests.serving_replay.mock_serving import (
...
async def _generate_openai_sse_stream(sample, chunk_size):
...
async def test_anthropic_streaming(sample, chunk_size):
    anthropic_serving = build_anthropic_serving(get_parser_cls(sample.parser_name))
    openai_sse_gen = _generate_openai_sse_stream(sample, chunk_size)
