...
"""Run the grammar-based Qwen3 XML tool parser through the common test suite.

Uses the same test data as test_qwen3xml_tool_parser.py but targets the
grammar-based parser registered as ``"qwen3_xml_grammar"``.  The existing
Qwen3 XML parser has 7 xfail streaming tests — the grammar parser should
pass them all.
"""
...
class TestGrammarQwen3XMLToolParser(ToolParserTests):
    @pytest.fixture
    def test_config(self) -> ToolParserTestConfig:
        return ToolParserTestConfig(
            parser_name="qwen3_xml_grammar",
