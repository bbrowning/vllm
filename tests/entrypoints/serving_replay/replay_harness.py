...
_DELEGATING_OLD_PAIRINGS: dict[str, tuple[str, str]] = {
    ...
    "nemotron_v3_engine": ("qwen3_coder", "nemotron_v3"),
    "qwen3_xml_engine": ("qwen3_xml", "qwen3"),
...
def make_mock_tokenizer(sample: ServingSample):
    ...
    Delegates to the parser engine test harness's ``make_mock_tokenizer``.
    """
    from tests.parser.engine.replay_harness import Sample as EngineSample

    engine_sample = EngineSample(
        id=sample.id,
    ...
    return _make_mock_tokenizer(engine_sample)
...
def get_parser_cls(parser_name: str, mode: str = "engine") -> type[Parser]:
